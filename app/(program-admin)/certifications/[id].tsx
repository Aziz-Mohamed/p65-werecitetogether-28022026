...
import { useIssueCertification } from '@/features/certifications/hooks/useIssueCertification';
...
export default function ProgramAdminCertificationDetailScreen() {
  ...
  const isSupervisorApproved = cert.status === 'supervisor_approved';
  const isIssued = cert.status === 'issued';
  const isQiraat = cert.program?.category === 'qiraat';
  ...
  const handleIssue = () => {
    Alert.alert(
      t('certifications.review.confirmIssue'),
      undefined,
      [
        { text: t('common.cancel'), style: 'cancel' },
        {
          text: t('certifications.review.issue'),
          onPress: () => {
            issueMutation.mutate(
              ...
              {
                onSuccess: () => {
                  Alert.alert(t('common.success'), t('certifications.review.issueSuccess'));
                  router.back();
                },
                onError: handleError,
              },
  ...
  const handleReturn = (notes: string) => {
    issueMutation.mutate(
      { certificationId: id!, action: 'reject', reviewNotes: notes },
      {
        onSuccess: () => {
  ...
  const handleReject = (notes: string) => {
    issueMutation.mutate(
      ...
      {
        onSuccess: () => {
          Alert.alert(t('common.success'), t('certifications.review.rejectSuccess'));
          router.back();
        },
        onError: handleError,
      },
  ...
  return (
    <Screen>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {isSupervisorApproved && isQiraat && (
          <View style={styles.chainSection}>
            <Text style={styles.sectionTitle}>{t('certifications.detail.chainOfNarration')}</Text>
            <TextInput
              style={styles.chainInput}
              multiline
              numberOfLines={3}
              placeholder={t('certifications.detail.chainOfNarrationPlaceholder')}
